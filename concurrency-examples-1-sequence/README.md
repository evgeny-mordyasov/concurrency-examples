Операция приращения `value++` не является [_атомарной_](../README.md#атомарные-операции) (atomic), то есть неделимой, а состоит из трех отдельных операций: чтения значения,
